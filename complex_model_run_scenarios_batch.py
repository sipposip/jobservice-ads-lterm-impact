...
plotdir = './plots_complexmodel_extended_exps/'
datadir = './data_complexmodel_extended_exps/'
...
parser.add_argument('--labormarket_bias', type=float)
parser.add_argument('--scale_factor', type=float, default=1/500)
parser.add_argument('--alpha_prot', type=float, default=2)
parser.add_argument('--delta_T_u', type=int, default=5)
parser.add_argument('--T_u_max', type=int, default=100)
parser.add_argument('--class_boundary', type=int, default=10)
parser.add_argument('--jobmarket_function_scale', type=float, default=6)
args = parser.parse_args()
modeltype = args.modeltype
labormarket_bias = args.labormarket_bias
class_boundary = args.class_boundary
...
delta_T_u = args.delta_T_u  # time lowpros are withdrawn from active group (default 5)
...
for config in configs:
    for i_run in range(n_runs):
        ...
        df_hist = pd.DataFrame(np.nan, index=np.arange(n_hist_max),
                               columns=['x1', 'x2', 'x_prot', 's_real', 'T_u', 'step'])
        ...
        for step in trange(n_spinup + tsteps):
            ...
            if step > n_spinup:
                ...
                tnr, fpr, fnr, tpr = metrics.confusion_matrix(classes_true_hist, classes_pred_hist,
                                                              normalize='all').ravel()
                ...
                tnr_priv, fpr_priv, fnr_priv, tpr_priv = metrics.confusion_matrix(classes_true_hist[idx_priv],
                                                                                  classes_pred_hist[idx_priv],
                                                                                  normalize='all').ravel()
                ...
                tnr_upriv, fpr_upriv, fnr_upriv, tpr_upriv = metrics.confusion_matrix(classes_true_hist[idx_upriv],
                                                                                      classes_pred_hist[idx_upriv],
                                                                                      normalize='all').ravel()
                ...
                precision = np.nan
                recall = np.nan
                tnr, fpr, fnr, tpr = np.nan, np.nan, np.nan, np.nan
                accur_priv = np.nan
                precision_priv = np.nan
                recall_priv = np.nan
                tnr_priv, fpr_priv, fnr_priv, tpr_priv = np.nan, np.nan, np.nan, np.nan
                accur_upriv = np.nan
                precision_upriv = np.nan
                recall_upriv = np.nan
                tnr_upriv, fpr_upriv, fnr_upriv, tpr_upriv = np.nan, np.nan, np.nan, np.nan
            ...
            _df['equal_opportunity'] = _df['tnr_priv'] - _df['tnr_upriv']
            _df['predictive_equality'] = _df['fpr_priv'] - _df['fpr_upriv']
            _df['coef1'] = coef1
            _df['coef2'] = coef2
        ...
        model_evolution[
            ['frac_highpros_pred', 'frac_highpros_true', 'frac_true_highpros_hist', 'frac_true_lowpros_hist',
